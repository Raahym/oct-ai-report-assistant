...
VKG_BACKEND_URL
CORNEAL_BACKEND_URL
CORNEAL_ULCER_BACKEND_URL
RETINA_DR_BACKEND_URL
RETINA_DR_GRADCAM_BACKEND_URL
RETINA_GLAUCOMA_BACKEND_URL
RETINA_HR_BACKEND_URL
RESEND_API_KEY
EMAIL_FROM
```

`SUPABASE_SERVICE_ROLE_KEY`, `AI_GATEWAY_SHARED_SECRET`, and all AI backend URLs must stay server-side only. Never expose model-service URLs or secrets as `NEXT_PUBLIC_*` variables.
